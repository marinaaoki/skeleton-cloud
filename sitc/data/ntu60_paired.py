...
class PairedNTU60(torch.utils.data.Dataset):
    ...
    def __init__(self, num_frames=75, num_joints=25,split="train", paired=False):
        ...
        adj_joint = scipy.sparse.coo_matrix((con_matrix, (j_pair_1, j_pair_2)), shape=(self.num_joints, self.num_joints)).toarray()
    ...
    def __getitem__(self, index):
        ...
        if self.paired:
            acs = Path(os.path.join(NTU60.dataset_path, f"pairings/NTU_RGBD_{cam}_{act}.txt")).read_text().splitlines()
            acs.remove(name)

            # Choose a random sample from aps
            paired_sample = np.random.choice(acs)

            # Load the skeleton data for the paired sample
            paired_keypoints_path = f"/media/ubi-lab-desktop/Extreme Pro/data/nturgb+d_skeletons/{paired_sample}.skeleton"
            paired_keypoint_samples = self.get_keypoint_samples(paired_keypoints_path)

            return {
                    "name": name,
                    "indices": index,
                    "keypoints": {
                        "original": keypoint_samples,
                        "dummy": paired_keypoint_samples
                    },
                    "camera": camera,
                    "action": action,
                    "person": person,
